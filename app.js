...
for (let key in env) {
    path = path ? path : '';
    let Type = key.substr(0,secureStringPerfix.length) === secureStringPerfix ? 'SecureString' : 'String';
    let Name = Type === 'SecureString' ? path + key.substr(secureStringPerfix.length) : path + key;
...
envArr.map(param => {
    SSM.putParameter(param, function(err){
        if (err){
            ...
            else console.log("\x1b[91m%s\x1b[0m", param.Name, err.code, ':', err.message);
        }
